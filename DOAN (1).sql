...
-- 13. Tìm sinh viên đạt loại A trong tất cả lớp mà họ tham gia
CREATE VIEW STUDENT_ALL_A_GRADES AS
SELECT
    s.`STU_NUM`,
    CONCAT(s.`STU_FNAME`, ' ', s.`STU_LNAME`) AS STUDENT_NAME,
    d.`DEPT_NAME`,
    COUNT(DISTINCT e.`CLASS_CODE`) AS TOTAL_CLASSES,
    SUM(CASE WHEN e.`ENROLL_GRADE` = 'A' THEN 1 ELSE 0 END) AS GRADE_A_CLASSES
FROM `STUDENT` s
JOIN `DEPARTMENT` d ON s.`DEPT_CODE` = d.`DEPT_CODE`
...
HAVING COUNT(DISTINCT e.`CLASS_CODE`) =
        SUM(CASE WHEN e.`ENROLL_GRADE` = 'A' THEN 1 ELSE 0 END)
ORDER BY s.`STU_LNAME`;

-- VIEW:
SELECT * FROM `STUDENT_ALL_A_GRADES`;
...
HAVING COUNT(DISTINCT c.`CLASS_CODE`) <= 2
...
-- 50. Liệt kê sinh viên có cùng khoa với cố vấn của họ
CREATE VIEW STUDENT_SAME_DEPARTMENT_AS_ADVISOR AS
SELECT
    s.`STU_NUM`,
    CONCAT(s.`STU_FNAME`, ' ', s.`STU_LNAME`) AS STUDENT_NAME,
    s.`DEPT_CODE`,
    CONCAT(p.`PROF_FNAME`, ' ', p.`PROF_LNAME`) AS ADVISOR_NAME
FROM `STUDENT` s
JOIN `PROFESSOR` p ON s.`ADVISOR_PROF_NUM` = p.`PROF_NUM`
WHERE s.`DEPT_CODE` = p.`DEPT_CODE`
ORDER BY s.`DEPT_CODE`, s.`STU_LNAME`;

-- VIEW:
SELECT * FROM `STUDENT_SAME_DEPARTMENT_AS_ADVISOR`;
...
INSERT INTO BRANCH (BRANCH_CODE, MANAGER_EMP_CODE, BRANCH_NAME, BRANCH_ADDRESS) VALUES
('BR_HN', 'EMP001', 'Chi nhánh Hà Nội', '12 Cầu Giấy, Hà Nội'),
('BR_DN', 'EMP002', 'Chi nhánh Đà Nẵng', '45 Nguyễn Văn Linh, Đà Nẵng'),
('BR_HCM', 'EMP003', 'Chi nhánh TP.HCM', '78 Lê Lợi, Quận 1, TP.HCM'),
('BR_HP', 'EMP004', 'Chi nhánh Hải Phòng', '102 Lạch Tray, Hải Phòng');
...
-- 40. Liệt kê xe chưa từng được phân tuyến
...
-- 41. Liệt kê tuyến đường chưa có xe chạy
CREATE VIEW ROUTES_WITHOUT_VEHICLE AS
SELECT
    r.`ROUTE_CODE`,
    r.`ROUTE_START_POINT`,
    r.`ROUTE_END_POINT`,
    r.`ROUTE_DISTANCE`
FROM `ROUTE` r
WHERE r.`ROUTE_CODE` NOT IN (
    SELECT rv.`ROUTE_CODE`
    FROM `ROUTE_VEHICLE` rv
)
ORDER BY r.`ROUTE_CODE`;

-- VIEW:
SELECT * FROM `ROUTES_WITHOUT_VEHICLE`;
...
ORDER BY r.`ROLE_NAME`, a.`USERNAME`;
